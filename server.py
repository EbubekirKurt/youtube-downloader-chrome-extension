...
DOWNLOAD_DIR = os.path.join(os.path.expanduser("~"), "Downloads", "YouTubeVideos")
...
def download_video(url, format_type, quality, video_id):
    try:
        ...
        # Dosya adını temizle ve abulauncher ekle
        safe_title = video_title.replace(' - YouTube', '').replace(' - YouTube Music', '')
        safe_title = ''.join(c for c in safe_title if c.isalnum() or c in (' ', '-', '_')).rstrip()
        safe_title = safe_title.replace(' ', '_')[:50] + '_abulauncher'
...
if __name__ == '__main__':
    print("🎥 YouTube Video İndirme API Server")
    print(f"📁 İndirme klasörü: {DOWNLOAD_DIR}")
    print("🌐 Server başlatılıyor: http://localhost:5000")
    print("📖 API dokümantasyonu:")
    print("  POST /api/info - Video bilgilerini al")
    print("  POST /api/download - Video indirmeyi başlat")
    print("  GET  /api/status/<video_id> - İndirme durumunu al")
    print("  GET  /api/downloads - İndirilen dosyaları listele")
    print("  GET  /health - Sağlık kontrolü")
    print("\n🚀 Extension için bu URL'yi kullanın: http://localhost:5000")
